...
Версия v1.2.1 совместима с сетевыми и кэшированными результатами IngameListMod: Global Demonlist располагается отдельной компактной строкой ниже его AREDL-позиции даже после повторного открытия уровня. Текстовые подписи `GLOBAL` и `AREDL` удалены; остаются центрированные кубок и число. Без IngameListMod строка учитывает все видимые монеты уровня. Также сохранены толстая рамка с прозрачными углами и англоязычное градиентное описание внутри Geode.
